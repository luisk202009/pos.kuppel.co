...
class ProductoServicioService {
  ...
  private productoServicio: Array<any> = [];

  constructor() {
    // Al iniciar la instancia solicitamos información al API
    if (localStorage.getItem('AccessToken') !== null)
    CurrentSessionObject.get(`${environment.api.app}/producto-servicio`)
    .then((json) => {
      this.productoServicio = json;
      this.dispatchOnLoad();
    });
  }
  ...
  get(id: string = null): any[] {
    if (id !== null) 
      for (const i in this.productoServicio){
        if (this.productoServicio[i]._id == id) return [this.productoServicio[i]];
      }
    else return this.productoServicio;
  }

  // Agrega un producto o servicio al arreglo (no lo guarda en el servidor) TOREVIEW
  add(productoServicio: any): number{
    return (this.productoServicio.push(productoServicio)-1);
  }

  // Permite actualizar un producto o servicio (no lo actualiza en el servidor)
  update(id: string, productoServicio: any): string{
    for (const i in this.productoServicio) {
      if (this.productoServicio[i]._id == id){
        this.productoServicio[i] = productoServicio;
        return i;
      }
    }
  }
  ...
  delete(id: string){
    ...
    for (const i in this.productoServicio) {
      if (this.productoServicio[i]._id != id)
        tmp.push(this.productoServicio[i]);
    }
    this.productoServicio = tmp;
  }
}

// Exportamos una instancia del servicio para que todas las importaciones 
// importen en mismo objeto sin recargar los datos
export const ProductoServicio: ProductoServicioService = new ProductoServicioService();